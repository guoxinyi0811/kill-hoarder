...
- **筛选规则不可放宽**：主视图永远只显示 `expired / urgent / soon`，不提供「显示全部」
  开关。一旦开这个口子，主视图就退化成普通列表，「打开只看到该管的」这个核心设计就没了。
  看全部是 P2「📦 家里有什么」的职责。
- 因此新增 `ok` / `untracked` 条目时列表不会有变化，必须靠 toast 回执告知（见验收）。
  toast 要带位置，例如「已保存：花椒 · 橱柜（目前无需处理）」——录入 L3 的动机就是怕忘了
  买过，回显位置能强化记忆。
...
**验收**：
- 新增后，若条目状态在筛选范围内则立即出现在列表（乐观更新，不等网络往返）；
  若不在范围内（`ok` / `untracked`），列表不变，但必须有 toast 确认，文案含名称与位置
- 标记消耗后条目消失且不可被普通视图查到
- 移动端单手可完成新增到保存全流程
- 列表中混入一条日期非法的数据时，其余条目正常渲染，该条显示为可点击的「⚠️ 日期数据异常」
- 表单无法提交格式非法或不存在的日期

**`quantity_level`**：是 `items` 的 not null 列。P1 的录入表单不暴露该字段，新增时走
数据库默认值 `'full'`。三档快速切换是 P2「📦 家里有什么」视图的职责。
...
> 「未记录开封日」提示归 **P2**，P1 未做。理由：本节整体是 P2 的种子数据，且未填开封日
> 又没有其他日期来源的护肤品会算成 `untracked`，被 P1 主视图的筛选规则挡在外面，
> 只有 P2 的「📦 家里有什么」视图才有地方显示它。
